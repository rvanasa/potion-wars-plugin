...
public class Language implements Listener {
	...
	public static String getLanguage() {
		if (PotionWarsPlugin.dataFolder == null)
			return null;
		//Get exact lang 
		String loc = getExactLanguage();
		File file = new File(PotionWarsPlugin.dataFolder, "/lang/" + loc + ".properties");
		if(file.exists()) {
			return loc;
		} else {
			return "en";
	...
	public static void setLanguage(String loc) {
		if (PotionWarsPlugin.dataFolder == null)
			return;
		try {
	    	File locfile = new File(PotionWarsPlugin.dataFolder, "config_locale.yml");
	...
	public static String getTranslation(String code, String locale) {
		...
		String transl = code;
		try {
			File locfile = new File(PotionWarsPlugin.dataFolder + "/lang/" + locale + ".properties");
			...
			try {
				File locfile = new File(PotionWarsPlugin.dataFolder + "/lang/en.properties");
				FileInputStream locStream = new FileInputStream(locfile);
				Properties locprop = new Properties();
				locprop.load(new InputStreamReader(locStream, Charset.forName("UTF-8")));
				transl = locprop.getProperty(code.toLowerCase());
